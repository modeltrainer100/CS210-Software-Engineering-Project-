...
def train_crop_model():
    """Train and return XGBoost crop recommendation model"""
    df = load_crop_data()
...
def show_crop_prediction():
    ...
    if predict_button:
        ...
        crop_classes = le.inverse_transform(model.classes_)
...
def show_disease_detection():
    ...
    st.markdown(f"# 🔬 {t['nav_disease'].split(' ')[1]} {t['nav_disease'].split(' ')[2]}") # This also needs to be translated
    st.markdown("Upload a leaf image for instant disease identification using deep learning CNN models.") # This needs translation
    
    st.markdown("### 📷 Upload Plant Leaf Image") # This needs translation
    
    st.markdown("""
    <div class="upload-section">
        <h4>📸 Image Upload Guidelines:</h4>
        <div style="text-align: left; display: inline-block;">
            ✓ Clear, well-lit leaf images<br>
            ✓ Focus on affected areas or symptoms<br>
            ✓ Supported formats: JPG, PNG, JPEG<br>
            ✓ Maximum size: 10MB
        </div>
    </div>
    """, unsafe_allow_html=True) # This whole block needs translation
    ...
    if uploaded_file is not None:
        ...
        with result_col:
            if st.button("🔍 Analyze for Diseases", type="primary", use_container_width=True):
                ...
                st.markdown(f"""
                <div class="prediction-result">
                    🎯 Predicted Disease: <strong>{predicted_disease}</strong><br>
                    📊 Confidence: <strong>{confidence:.1f}%</strong>
                </div>
                """, unsafe_allow_html=True)

                if predicted_disease != "Healthy":
                    st.error("❗ Your plant may be diseased. Please consult a professional for confirmation.")
                else:
                    st.balloons()
